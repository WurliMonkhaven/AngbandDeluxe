...
static const struct { int id; const char *name,*kind,*help; } deluxe_status_metadata[]={
...
static cJSON *deluxe_statuses(void)
{
 ...
 for(i=0;i<TMD_MAX;++i) if(player->timed[i]>0 || i==TMD_FOOD) {
  ...
  for(j=0;j<N_ELEMENTS(deluxe_status_metadata);++j) if(deluxe_status_metadata[j].id==i) {
   name=deluxe_status_metadata[j].name; kind=deluxe_status_metadata[j].kind; help=deluxe_status_metadata[j].help; break;
  }
...
/* Command requires an actual controlled monster; it is not a standalone buff. */
static bool deluxe_debug_status_allowed(int idx)
{
 return idx>=0 && idx<TMD_MAX && idx!=TMD_COMMAND;
}
static cJSON *deluxe_debug_status_catalog(void)
{
 cJSON *out=cJSON_CreateArray(); size_t i;
 for(i=0;i<N_ELEMENTS(deluxe_status_metadata);++i) {
  int idx=deluxe_status_metadata[i].id;
  const struct timed_grade *grade=timed_effects[idx].grade; cJSON *row,*grades;
  if(!deluxe_debug_status_allowed(idx)) continue;
  row=cJSON_CreateObject(); grades=cJSON_CreateArray();
  string(row,"id",timed_effects[idx].name);
  string(row,"name",idx==TMD_FOOD?"Nourishment":deluxe_status_metadata[i].name);
  string(row,"description",deluxe_status_metadata[i].help);
  string(row,"counter_kind",idx==TMD_FOOD?"nourishment":idx==TMD_CUT || idx==TMD_STUN?"severity":"duration");
  while(grade) {
   if(grade->name && *grade->name) {
    cJSON *g=cJSON_CreateObject(); string(g,"name",grade->name); number(g,"maximum",grade->max); cJSON_AddItemToArray(grades,g);
   }
   if(!grade->next) { number(row,"maximum",grade->max); break; }
   grade=grade->next;
  }
  cJSON_AddItemToObject(row,"grades",grades); cJSON_AddItemToArray(out,row);
 }
 return out;
}
