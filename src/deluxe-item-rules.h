...
static bool deluxe_kind_ignore_allowed(const struct object *obj)
{
 return ignore_tval(obj->tval) && (!obj->artifact || !object_flavor_is_aware(obj));
}
static void deluxe_item_preferences(cJSON *record,const struct object *obj)
{
 ...
 string(p,"kind_label",label); json_bool(p,"kind_allowed",deluxe_kind_ignore_allowed(obj));
...
static void deluxe_rule_row(cJSON *rows,const char *type,int index,int subtype,const char *label,const char *value)
{
...
static cJSON *deluxe_item_rules(void)
{
 ...
 for(i=1;i<(int)item_handle_count;++i) {
  ...
  if(obj && obj->known && (obj->known->notice&OBJ_NOTICE_IGNORE)) {
   ...
   deluxe_rule_row(rows,"instance",i,0,label,"Ignore this item");
  }
 }
 for(i=0;i<z_info->k_max;++i) {
  ...
  if(kind_is_ignored_aware(kind)) deluxe_rule_row(rows,"kind",i,1,label,"Ignore when known");
  if(kind_is_ignored_unaware(kind)) deluxe_rule_row(rows,"kind",i,0,label,"Ignore when unknown");
  if(kind->note_aware) deluxe_rule_row(rows,"auto",i,1,label,quark_str(kind->note_aware));
  if(kind->note_unaware) deluxe_rule_row(rows,"auto",i,0,label,quark_str(kind->note_unaware));
 }
 for(i=0;i<ITYPE_MAX;++i) if(ignore_level[i]!=IGNORE_NONE)
  deluxe_rule_row(rows,"quality",i,0,ignore_name_for_type(i),quality_values[ignore_level[i]].name);
 for(i=0;i<z_info->e_max;++i) if(e_info[i].name) for(j=0;j<ITYPE_MAX;++j) if(ego_is_ignored(i,j)) {
  ...
  deluxe_rule_row(rows,"ego",i,j,label,"Ignore");
 }
 cJSON_AddItemToObject(out,"rules",rows); return out;
}
static void deluxe_clear_rule(const cJSON *r)
{
