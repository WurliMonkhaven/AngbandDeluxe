...
static void deluxe_capture_view(cJSON *state_record)
{
 ...
 if ((!ready && !textui_message_pending && !target_ui_current && !textui_aiming && !textui_direction && !item_choice_objects) || active_prompt || screen_save_depth || !streq(phase,"playing") || !deluxe_cells) return;
