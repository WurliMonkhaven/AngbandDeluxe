...
struct StatusEffects {
 ...
 static void draw(const json &player) {
  const auto effects=active(player); if(effects.empty()) return;
  ...
  for(const auto &effect:effects) {
   ...
   const float badge_width=std::min(width,ImGui::CalcTextSize(name.c_str()).x+3*pad);
   ...
   const auto text_size=ImGui::CalcTextSize(name.c_str(),nullptr,false,text_width);
   ...
   draw->AddText(ImGui::GetFont(),ImGui::GetFontSize(),ImVec2(a.x+1.5f*pad,a.y+pad*.5f),ImGui::GetColorU32(accent),name.c_str(),nullptr,text_width);
   draw->PopClipRect(); ImGui::Dummy(size);
   if(ImGui::IsItemHovered()) {
    ...
    ImGui::TextDisabled("%s",kind=="harm"?"Harmful effect":kind=="mixed"?"Benefits and drawbacks":kind=="benefit"?"Beneficial effect":"Active effect");
