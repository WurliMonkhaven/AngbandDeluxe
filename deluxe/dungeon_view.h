...
inline const nlohmann::json *dungeon_view(const nlohmann::json &state) {
 if(state.value("phase","")!="playing" || (state.value("readiness","")!="ready" && !state.value("native_prompt",false) && !state.value("message_pending",false) && !state.contains("targeting") && !state.value("aiming",false) && !state.value("direction_prompt",false) && !state.value("item_selection",false) && !state.value("spell_selection",false))) return nullptr;
