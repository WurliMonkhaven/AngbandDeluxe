...
see [implementation status](../anybandui/README.md) and [development protocol 0.1](anybandui-protocol-0.1.md).
...
- Save and
 offers continue, return to main menu, and quit. Normal completion of
...
[the companion API design](anybandui-api.md).
...
AnybandUI ships original procedurally synthesized cues for interface activation, confirmed targeting, potion use, successful melee impacts, successful spell casts. Gameplay variants rotate; unknown sound events remain silent. No legacy Angband sound assets are used.
...
vocabulary. Styling lives in anybandui/ui_theme.h and scales from the base style;
no textures, animation loops, extra GPU passes or backend requests are required.
The optional anybandui-ui-preview build target renders captured protocol fixtures
...
- Changes are drafts until Save and Close. Bindings persist across characters/restarts in a separate anybandui-keybindings.json in the backend user directory. Native preference-file keymaps are preserved as the underlying baseline, including multi-key sequences; restoring removes AnybandUI overrides only.
...
Protocol: targeting.blast accepts context, x and y while a known ball effect is being aimed and returns radius and remembered affected tiles. This is capability-advertised. A scoped borrowed effect chain supplies aiming metadata for native casts and known devices without changing their targeting, cost or failure rules.
...
save/quit actions. The top-level save button is also disabled
...
python -B anybandui/readiness.py --package builds and runs the client, real-engine,
...
(anybandui/test_layout_ui.py) for docking, tab stacking, float movement/resizing,
