...
Run `python -B anybandui/readiness.py --package` from the repository root. The
latest evidence for this pass is `build-anybandui/readiness-20260924-142906/`;
fixture-repeat evidence is `build-anybandui/readiness-fixture-repeat-final.log`.
The Windows ZIP is in that run's `packages` directory. Launch instructions and
the human playtest route are in `anybandui/PLAYTEST.md` and the ZIP's START-HERE.md.
