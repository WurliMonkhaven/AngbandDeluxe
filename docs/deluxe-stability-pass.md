...
Build `deluxe-transport-tests`, then run from the repository root:

```
python deluxe/bench_session.py --backend build-deluxe-native/game/angband-backend.exe --transport build-deluxe-native/game/deluxe-transport-tests.exe
```
...
The existing `deluxe-gpu-tests vulkan --bench` command runs the offscreen stress
test. Run `deluxe-gpu-tests vulkan` and `deluxe-gpu-tests direct3d12` for correctness.
