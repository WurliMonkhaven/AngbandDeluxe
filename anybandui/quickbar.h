...
struct Quickbar {
 ...
 void drag_source(const json &binding,int source=-1) {
  ...
  ImGui::SetDragDropPayload("ANYBANDUI_QUICKBAR",wire.data(),wire.size(),ImGuiCond_Once);
 ...
 void drop_target(int slot) {
  ...
  if(const auto *payload=ImGui::AcceptDragDropPayload("ANYBANDUI_QUICKBAR")) {
 ...
 int draw(Connection &c) {
  ...
  for(int i=0;i<10;++i) {
   ...
   AnybandUITheme::corners(draw,border_min,border_max,ImGui::GetColorU32(usable?AnybandUITheme::green():ImGui::GetStyleColorVec4(ImGuiCol_TextDisabled)),h*.12f);
