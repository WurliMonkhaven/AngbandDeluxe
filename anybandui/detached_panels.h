...
struct DetachedPanels {
 ...
 bool open(UI &ui,int p) {
  ...
  AnybandUITheme::apply(); w.fonts.load(font_folder);
 ...
 void draw(UI &ui) {
  ...
  for(int p=0;p<WorkspaceLayout::Count;++p) {
   ...
   ImGui::GetStyle()=ui.base_style; AnybandUITheme::configure(ui.theme_settings);
  ...
  ImGui::SetCurrentContext(main_context); AnybandUITheme::current=ui.theme_settings;
 }
};
