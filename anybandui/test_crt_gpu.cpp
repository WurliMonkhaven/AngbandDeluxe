...
int main(int argc,char **argv) {
 try {
  ...
  auto render=[&](bool lit,bool overlay=false) {
   ...
   const auto uploads_before=AnybandUIGpuGetUploadStats().uploads;
   renderer.render(cmd,target,texture.width,texture.height,ImGui::GetDrawData(),frame);
   check(AnybandUIGpuGetUploadStats().uploads==uploads_before+1,"UI was uploaded more than once in a frame");
