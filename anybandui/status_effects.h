...
struct StatusEffects {
 ...
 static float height(const json &player,float width,bool headings=true) {
  ...
  const float title=headings?AnybandUITheme::section_height()+style.ItemSpacing.y:0;
 ...
 static void draw(const json &player,bool *open_spells=nullptr,float study_flash=0) {
  ...
  for(const auto &effect:effects) {
   ...
   const auto background=light?AnybandUITheme::mix(surface,accent,.16f):ImVec4(accent.x*.16f,accent.y*.16f,accent.z*.16f,1);
   const auto border=light?AnybandUITheme::mix(surface,text_ink,.45f):ImVec4(accent.x,accent.y,accent.z,.5f);
   const auto label=badge_label(effect);
   const float badge_width=std::min(width,ImGui::CalcTextSize(label.c_str()).x+3*pad);
