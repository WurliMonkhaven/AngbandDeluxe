...
static void anybandui_sound_event(int type)
{
...
static void anybandui_target_selected(void)
{
...
#include "anybandui-spells.h"
#include "anybandui-item-rules.h"
static cJSON *item_record(const struct object *o, const char *location, int index)
{
 ...
 if(item_is_available((struct object *)o)) anybandui_item_preferences(j,o);
 anybandui_book_record(j,o);
...
#include "anybandui-character.h"
#include "anybandui-options.h"
#include "anybandui-tuning.h"
#include "anybandui-keybindings.h"
#include "anybandui-journal.h"
#include "anybandui-run.h"
#include "anybandui-save-summary.h"
#include "anybandui-status.h"
#include "anybandui-knowledge.h"
static int anybandui_blast_radius(void);
#include "anybandui-tiles.h"
#include "anybandui-view.h"
#include "anybandui-blast.h"
#include "anybandui-projectiles.h"
#include "anybandui-motion.h"
#include "anybandui-compare.h"
#include "anybandui-travel.h"
#include "anybandui-store.h"

static void publish(void);
static void pump(void);
#include "anybandui-birth.h"
static cJSON *capture(void)
{
 ...
 if(birth_active) cJSON_AddItemToObject(s,"birth",anybandui_birth_record());
 if (character_generated && player && cave) {
  ...
  cJSON_AddItemToObject(p,"statuses",anybandui_statuses());
  cJSON_AddItemToObject(s, "player", p);
  anybandui_capture_view(s);
  anybandui_capture_terrain_actions(s);
  ...
  counter(map,"level_id",anybandui_level);
 ...
 if (active_store) anybandui_capture_store(s, items, &index);
 for (i = 0; i < messages_num() && i < 200; ++i) {
  ...
  string(m,"group",anybandui_message_group(message_type(i)));
 ...
 if(streq(phase,"dead") && !run_report) run_report=anybandui_run_record(s);
...
static void publish(void)
{
 ...
 anybandui_motion_publish();
 anybandui_knowledge_publish();
}
...
static void pump(void)
{
 ...
 if (streq(method, "hello")) {
  ...
  out = cJSON_Parse("{\"protocol\":{\"major\":0,\"minor\":1},\"engine\":{\"id\":\"org.angband.angband\",\"version\":\"4.2.6-anybandui-dev\",\"save_compatibility\":\"angband-4.2.6\"},\"capabilities\":{\"state.player\":1,\"state.items\":1,\"state.map\":1,\"state.monsters\":1,\"state.messages\":1,\"commands\":1,\"prompts.basic\":1,\"prompts.items\":1,\"spells\":1,\"audio.events\":1,\"session.replay\":1,\"run.summary\":1,\"journal\":1,\"keybindings\":1,\"options\":1,\"tuning\":1,\"knowledge.watch\":1,\"knowledge\":1,\"item.rules\":1,\"item.compare\":1,\"interaction.birth\":1,\"interaction.store\":1,\"interaction.inventory\":1,\"targeting.blast\":1,\"terminal.fallback\":1,\"presentation.dungeon\":1,\"presentation.tiles\":1,\"presentation.viewport\":1,\"presentation.camera\":1,\"interaction.targeting\":1,\"interaction.route\":1,\"interaction.mouse\":1,\"interaction.pickup\":1,\"interaction.terrain\":1},\"max_frame_bytes\":1048576}");
 ...
 if(streq(method,"tuning.get") || streq(method,"tuning.set")) tuning_request(id,method,p);
  ...
  if(!cJSON_IsNumber(choice) || choice->valuedouble!=mode || mode<0 || mode>6 || !get_graphics_mode(mode)) error(id,"invalid_argument","Unknown tileset.");
   ...
   anybandui_tiles_id=mode; anybandui_tiles_failed=false;
   if(character_generated) publish();
   response(id,cJSON_CreateObject());
  }
 }
 else if (streq(method, "dungeon.viewport")) {
  cJSON *w=cJSON_GetObjectItem(p,"width"), *h=cJSON_GetObjectItem(p,"height");
  const int width=num(p,"width",-1),height=num(p,"height",-1);
  if(!cJSON_IsNumber(w) || !cJSON_IsNumber(h) || w->valuedouble!=width || h->valuedouble!=height ||
     !((width==0 && height==0) || (width>=9 && width<=240 && height>=5 && height<=128)))
   error(id,"invalid_argument","Expected a 9-240 by 5-128 viewport, or zero for automatic sizing.");
  else if(frame_limit<CAMERA_FRAME_LIMIT) error(id,"invalid_argument","Viewport sizing requires a 4 MiB frame budget.");
  else if(character_generated && (!ready || active_prompt || !streq(phase,"playing"))) error(id,"busy","Change viewport size during normal play.");
  else {
   anybandui_view_width=width; anybandui_view_height=height; anybandui_view_initialized=false;
   if(character_generated) publish();
   response(id,cJSON_CreateObject());
  }
 }
  ...
  if(!cJSON_IsBool(cJSON_GetObjectItem(p,"enabled"))) error(id,"invalid_argument","Expected an enabled flag.");
   ...
   anybandui_full_map=cJSON_IsTrue(cJSON_GetObjectItem(p,"enabled"));
   if(character_generated) publish();
   response(id,cJSON_CreateObject());
  }
 }
 else if (streq(method, "state.get")) response(id, snapshot ? cJSON_CreateObjectReference(snapshot->child) : cJSON_CreateObject());
 else if(streq(method,"knowledge.unwatch")) { anybandui_knowledge_unwatch(); response(id,cJSON_CreateObject()); }
 else if(streq(method,"knowledge.list") || streq(method,"knowledge.get")) anybandui_knowledge_request(id,method,p);
 else if(streq(method,"item.rules.list")) {
  if(!character_generated) error(id,"wrong_phase","Start a character first.");
  else response(id,anybandui_item_rules());
 }
  ...
  if(streq(method,"item.rules.clear")) {
   ...
   if(streq(str(p,"operation"),"ignore_item") && cJSON_IsBool(cJSON_GetObjectItem(p,"enabled"))) {
    ...
    changed=true;
   } else if(streq(str(p,"operation"),"ignore_kind") && anybandui_kind_ignore_allowed(obj) && cJSON_IsBool(cJSON_GetObjectItem(p,"enabled"))) {
  ...
  if(!obj) error(id,"stale_handle","Select a current item.");
   ...
   cJSON_AddItemToObject(out,"options",anybandui_compare(obj)); response(id,out);
  }
 }
  ...
  while (got_savefile(&g)) {
   ...
   string(j, "id", d->fnam); string(j, "description", d->desc); anybandui_save_summary(j,d->fnam,d->desc); cJSON_AddItemToArray(out, j);
  }
  cleanup_savefile_getter(g); response(id, out);
 } else if (!streq(str(p, "session_id"), "session-1")) error(id, "wrong_session", "Stale session.");
 else if(streq(method,"keybindings.get") || streq(method,"keybindings.set")) { anybandui_bindings_request(id,method,p); }
 else if (streq(method,"store.buy") || streq(method,"store.sell") || streq(method,"store.leave")) {
  anybandui_store_request(id, method, p);
  ...
  if(!character_generated) error(id,"wrong_phase","Start a character first.");
  else response(id,anybandui_journal(false));
 } else if (streq(method,"targeting.blast")) {
  anybandui_blast_preview(id,p);
 } else if (streq(method,"dungeon.route")) {
  anybandui_route_preview(id,p);
  ...
  cmd_code command=anybandui_terrain_command(grid);
  ...
  else if(!cJSON_IsNumber(jx) || !cJSON_IsNumber(jy) || jx->valuedouble!=jx->valueint || jy->valuedouble!=jy->valueint || command==CMD_NULL || !streq(str(p,"action"),anybandui_terrain_action(command))) error(id,"invalid_argument","That action is not available on this terrain.");
  else {
   travel_command=command; travel_grid=grid; travel_level=anybandui_level; travel_stage=TRAVEL_START;
   anybandui_travel_begin(grid,travel_command);
   ready=false; Term_keypress(ESCAPE,0); response(id,cJSON_CreateObject());
  }
  ...
  else if(!cJSON_IsNumber(jx) || !cJSON_IsNumber(jy) || jx->valuedouble!=jx->valueint || jy->valuedouble!=jy->valueint || !anybandui_pickup_observed(grid))
   error(id,"invalid_argument","No observed item at that location.");
  else {
   travel_command=CMD_PICKUP; travel_grid=grid; travel_level=anybandui_level; travel_stage=TRAVEL_START;
   anybandui_travel_begin(grid,travel_command);
   ready=false; Term_keypress(ESCAPE,0); response(id,cJSON_CreateObject());
  }
  ...
  if(active_prompt || !streq(str(p,"context"),context_text)) error(id,"stale_revision","Input context changed.");
   ...
   error(id,"busy","Return to normal play before moving.");
          ...
          (!anybandui_full_map && (grid.x<terminal.offset_x || grid.y<terminal.offset_y || grid.x>=terminal.offset_x+SCREEN_WID || grid.y>=terminal.offset_y+SCREEN_HGT)))
   ...
   if(!mods && !loc_eq(player->grid,grid)) anybandui_travel_begin(grid,CMD_WALK);
   if(exit_look || anybandui_full_map) {
    ready=false;
  ...
  if(active_prompt || !streq(str(p,"context"),context_text)) error(id,"stale_revision","Input context changed.");
   ...
   if(target_ui_current) { target_ui_select(grid,confirm); response(id,cJSON_CreateObject()); }
    ...
    if((!anybandui_full_map && (grid.x<terminal.offset_x || grid.y<terminal.offset_y || grid.x>=terminal.offset_x+SCREEN_WID || grid.y>=terminal.offset_y+SCREEN_HGT)))
    ...
    } else if(anybandui_full_map && textui_aiming) {
     textui_aim_at(grid); response(id,cJSON_CreateObject());
    } else if(anybandui_full_map && textui_direction) {
   ...
   else if(key=='t' && !target_ui_current->can_confirm && !anybandui_look_location()) error(id,"invalid_argument","This selection cannot be confirmed.");
   else { anybandui_target_key(key); response(id,cJSON_CreateObject()); }
  }
  ...
  else { if(travel_display_active) anybandui_travel_feedback("Travel cancelled by input",false,true); travel_stage=TRAVEL_IDLE; anybandui_target_key(key); response(id, cJSON_CreateObject()); }
   ...
   if (i == N_ELEMENTS(commands)) error(id, "invalid_argument", "Unknown command.");
    ...
    if(*str(p,"spell")) {
     ...
     if((!study && !streq(commands[i].id,"core.cast")) ||
        (pending_spell=anybandui_requested_spell(pending_item,str(p,"spell"),study))<0) {
  ...
  else response(id,anybandui_options());
  ...
  else if(!anybandui_options_apply(cJSON_GetObjectItem(p,"values")))
   error(id,"invalid_argument","Invalid option, value or range. No options were changed.");
  else {
   options_redraw=true; ready=false;
   response(id,anybandui_options()); Term_keypress(ESCAPE,0);
  }
...
static void combat_feedback(game_event_type type, game_event_data *data, void *user)
{
 ...
 json_bool(v,"player",data->combat.player); counter(v,"level_id",anybandui_level);
...
static errr get_command(cmd_context context)
{
 ...
 if(anybandui_travel_continue()) { ready=false; return 0; }
 if(click_after_look) {
...
static void lifecycle(game_event_type type, game_event_data *data, void *user)
{
 ...
 else if (type == EVENT_ENTER_WORLD) { phase = "playing"; anybandui_bindings_init(); }
...
{
 const char *data = "lib", *user = "anybandui-user"; int i;
#ifdef WINDOWS
 ...
 plog_aux = log_hook; ANGBAND_SYS = "anybandui";
 ...
 if(use_native_birth) birth_interact_hook=anybandui_birth_session;
 get_check_hook = check_hook; get_string_hook = string_hook; get_quantity_hook = quantity_hook;
 map_visual_hook = anybandui_observe_cell; map_visual_reset_hook = anybandui_reset_view;
 original_get_item = get_item_hook; get_item_hook = item_hook;
 get_spell_hook = anybandui_get_spell; get_spell_from_book_hook = anybandui_spell_choose;
 book_browse_hook = anybandui_browse_book;
 inventory_browse_hook = anybandui_inventory_browser;
 equipment_browse_hook = anybandui_equipment_browser;
 store_interact_hook = anybandui_store_session;
 cmd_get_hook = get_command;
 sound_event_hook=anybandui_sound_event;
 target_selected_hook=anybandui_target_selected;
 event_add_handler(EVENT_COMBAT_FEEDBACK, combat_feedback, NULL);
 event_add_handler(EVENT_ACTOR_MOTION, anybandui_motion_event, NULL);
 event_add_handler(EVENT_BOLT, anybandui_projectile, NULL);
 event_add_handler(EVENT_MISSILE, anybandui_projectile, NULL);
 event_add_handler(EVENT_EXPLOSION, anybandui_projectile, NULL);
 ...
 event_add_handler(EVENT_INPUT_FLUSH,anybandui_travel_event,NULL);
 event_add_handler(EVENT_AUTOPICKUP_BEGIN,anybandui_travel_event,NULL);
 event_add_handler(EVENT_AUTOPICKUP_END,anybandui_travel_event,NULL);
 event_add_handler(EVENT_NEW_LEVEL_DISPLAY,anybandui_travel_event,NULL);
 event_add_handler(EVENT_NEW_LEVEL_DISPLAY,anybandui_journal_level,NULL);
 event_add_handler(EVENT_ENTER_STORE,anybandui_travel_event,NULL);
 event_add_handler(EVENT_ENTER_DEATH,anybandui_travel_event,NULL);
 ...
 anybandui_tiles_free(); close_graphics_modes();
