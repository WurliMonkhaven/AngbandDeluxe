...
static cJSON *anybandui_spell_record(int index, bool available)
{
...
static void anybandui_book_record(cJSON *record, const struct object *obj)
{
 ...
 for(i=0;i<book->num_spells;++i) cJSON_AddItemToArray(spells,anybandui_spell_record(book->spells[i].sidx,available));
...
static int anybandui_requested_spell(const struct object *obj, const char *id, bool study)
{
 const struct class_book *book;
 int i;
...
static int anybandui_spell_choose(struct player *p, const char *verb, struct object *obj,
 const char *error_text, bool (*filter)(const struct player *,int))
{
 ...
 for(i=0;i<book->num_spells;++i) {
  ...
  choice=anybandui_spell_record(index,item_is_available(obj));
...
static int anybandui_get_spell(struct player *p, const char *verb, item_tester book_filter,
 ...
 const char *spell_error, struct object **rtn_book)
{
 ...
 return anybandui_spell_choose(p,verb,book,spell_error,filter);
}

static void anybandui_browse_book(const struct object *obj)
{
 spell_browsing=true;
 anybandui_spell_choose(player,"Browse",(struct object *)obj,"You cannot browse that.",spell_okay_to_browse);
