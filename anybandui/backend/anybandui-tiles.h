...
static void anybandui_tiles_free(void)
{
 ...
 memset(b,0,sizeof(*b)); anybandui_tiles_loaded=0;
}
static bool anybandui_tiles_prepare(void)
{
 graphics_mode *mode=get_graphics_mode(anybandui_tiles_id), *old_mode=current_graphics_mode;
 int old_graphics=use_graphics, max_flavor=0;
 uint8_t old_proj_a[PROJ_MAX][BOLT_MAX]; wchar_t old_proj_c[PROJ_MAX][BOLT_MAX];
 struct anybandui_visual_bank *b=&anybandui_tile_bank;
 if(!anybandui_tiles_id || !mode || anybandui_tiles_failed) return false;
 if(anybandui_tiles_loaded==anybandui_tiles_id) return true;
...
#undef ALLOC_VISUAL
 ...
 anybandui_visual_swap(b); use_graphics=anybandui_tiles_id; current_graphics_mode=mode;
 reset_visuals(false);
 anybandui_tiles_failed=!process_tile_pref_file(mode->path,mode->pref);
 anybandui_visual_swap(b); use_graphics=old_graphics; current_graphics_mode=old_mode;
 memcpy(proj_to_attr,old_proj_a,sizeof(old_proj_a)); memcpy(proj_to_char,old_proj_c,sizeof(old_proj_c));
 if(anybandui_tiles_failed) return false;
 anybandui_tiles_loaded=anybandui_tiles_id; return true;
}
static void anybandui_capture_tiles(cJSON *view,int ox,int oy,int width,int height)
{
 if(!anybandui_tiles_prepare()) return;
 graphics_mode *old_mode=current_graphics_mode;
 int old_graphics=use_graphics;
 cJSON *rows=cJSON_CreateArray();
 anybandui_visual_swap(&anybandui_tile_bank);
 use_graphics=anybandui_tiles_id; current_graphics_mode=get_graphics_mode(anybandui_tiles_id);
 ...
 anybandui_visual_swap(&anybandui_tile_bank); use_graphics=old_graphics; current_graphics_mode=old_mode;
 number(view,"tileset",anybandui_tiles_id); cJSON_AddItemToObject(view,"tiles",rows);
}
