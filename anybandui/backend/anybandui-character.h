...
static void anybandui_sheet_row(void *user, const char *group, const char *label, const char *value, int attr)
{
...
static void anybandui_character_sheet(cJSON *p)
{
 ...
 int i;
 character_sheet_rows(anybandui_sheet_row,rows);
 string(sheet,"history",player->history?player->history:"");
 for(i=0;i<STAT_MAX;++i) {
  cJSON *row=cJSON_CreateObject();
  string(row,"label",stats[i]); anybandui_sheet_stat(row,"base",player->stat_max[i]);
  anybandui_sheet_stat(row,"current",player->state.stat_use[i]);
  anybandui_sheet_stat(row,"best",player->state.stat_top[i]);
