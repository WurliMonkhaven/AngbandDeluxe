...
static bool anybandui_binding_key(int key)
{
 return (key>=KC_F1 && key<=KC_F12) ||
  (key>=1 && key<=26 && key!=8 && key!=9 && key!=10 && key!=13) ||
  (key>=33 && key<=126 && !(key>='0' && key<='9') && key!='\\' && key!='^');
}
static bool anybandui_binding_exposed(int group,const struct cmd_info *cmd)
{
 return (cmd->cmd || cmd->hook) && cmd->key[0] &&
  (group<5 || (cmd->key[0]<128 && strchr(";.,+psn:V",cmd->key[0])) || cmd->key[0]==KTRL('L'));
}
static struct cmd_info *anybandui_binding_command(int code)
{
 int group; size_t i;
 for(group=0;group<6;++group) for(i=0;i<cmds_all[group].len;++i) {
  struct cmd_info *cmd=&cmds_all[group].list[i];
  if((int)cmd->key[0]==code && anybandui_binding_exposed(group,cmd)) return cmd;
 }
 return NULL;
}
static struct keypress anybandui_binding_press(int code)
{ struct keypress key=KEYPRESS_NULL; key.type=EVT_KBRD; key.code=code; return key; }
static void anybandui_binding_label(cJSON *out,const char *field,int code)
{
 char text[80]; struct keypress keys[2]={KEYPRESS_NULL,KEYPRESS_NULL};
 keys[0]=anybandui_binding_press(code); keypress_to_text(text,sizeof(text),keys,true); string(out,field,text);
}
static bool anybandui_bindings_valid(const cJSON *rows)
{
 ...
 cJSON_ArrayForEach(row,rows) {
  ...
  if(!cJSON_IsNumber(mode) || mode->valuedouble!=mode->valueint || mode->valueint<0 || mode->valueint>1 ||
     !cJSON_IsNumber(key) || key->valuedouble!=key->valueint || !anybandui_binding_key(key->valueint) ||
     !cJSON_IsNumber(cmd) || cmd->valuedouble!=cmd->valueint || !anybandui_binding_command(cmd->valueint)) return false;
  if(used[mode->valueint][key->valueint]) return false;
  used[mode->valueint][key->valueint]=true;
 }
 return true;
}
static void anybandui_bindings_install(const cJSON *rows)
{
 const cJSON *row;
 ...
 cJSON_ArrayForEach(row,keybinding_overrides) {
  ...
  keymap_remove(mode,anybandui_binding_press(key));
  if(keybinding_base[mode][key][0].type)
   keymap_add(mode,anybandui_binding_press(key),keybinding_base[mode][key],keybinding_base_user[mode][key]);
 }
 cJSON_ArrayForEach(row,rows) {
  int mode=num((cJSON *)row,"mode",0),key=num((cJSON *)row,"key",0);
  struct cmd_info *cmd=anybandui_binding_command(num((cJSON *)row,"command",0));
  struct keypress action[2]={KEYPRESS_NULL,KEYPRESS_NULL};
  action[0]=anybandui_binding_press(cmd->key[mode]);
  /* Keymaps expand through the normal input path without recursively remapping. */
  keymap_add(mode,anybandui_binding_press(key),action,true);
 }
 cJSON_Delete(keybinding_overrides); keybinding_overrides=cJSON_Duplicate(rows,true);
 ++keybinding_revision;
}
static void anybandui_bindings_init(void)
{
 int mode,key; char path[1024],buffer[65536]; ang_file *file; cJSON *rows;
 if(keybinding_overrides) return;
 for(mode=0;mode<2;++mode) for(key=1;key<256;++key) if(anybandui_binding_key(key)) {
  const struct keypress *action=keymap_find(mode,anybandui_binding_press(key)); size_t i;
  if(action) for(i=0;i<KEYMAP_ACTION_MAX && action[i].type;++i) keybinding_base[mode][key][i]=action[i];
  keybinding_base_user[mode][key]=keymap_is_user(mode,anybandui_binding_press(key));
 }
 keybinding_overrides=cJSON_CreateArray();
 path_build(path,sizeof(path),ANGBAND_DIR_USER,"anybandui-keybindings.json");
 ...
 if(anybandui_bindings_valid(rows)) anybandui_bindings_install(rows);
 else fprintf(stderr,"AnybandUI keybindings could not be loaded; keeping native bindings.\n");
 cJSON_Delete(rows);
}
static cJSON *anybandui_bindings_get(void)
{
 ...
 for(group=0;group<6;++group) for(i=0;i<cmds_all[group].len;++i) {
  ...
  if(!anybandui_binding_exposed(group,cmd)) continue;
  ...
  for(mode=0;mode<2;++mode) { anybandui_binding_label(row,mode?"rogue":"original",cmd->key[mode]); number(row,mode?"rogue_key":"original_key",cmd->key[mode]); }
 ...
 for(mode=0;mode<2;++mode) for(key=1;key<256;++key) if(anybandui_binding_key(key)) {
  ...
  number(row,"mode",mode); number(row,"key",key); anybandui_binding_label(row,"label",key);
  string(row,"default_action",base[0].type?"Existing Angband keymap":label); cJSON_AddItemToArray(keys,row);
 }
 return out;
}
static bool anybandui_bindings_save(const cJSON *rows)
{
...
static void anybandui_bindings_request(const char *id,const char *method,cJSON *params)
{
 ...
 if(streq(method,"keybindings.get")) { response(id,anybandui_bindings_get()); return; }
 ...
 else if(!anybandui_bindings_valid(rows)) error(id,"invalid_argument","Invalid binding, reserved key or duplicate trigger. No bindings changed.");
 else if(!anybandui_bindings_save(rows)) error(id,"io_error","Could not save keybindings. Existing bindings are unchanged.");
 else { anybandui_bindings_install(rows); response(id,anybandui_bindings_get()); }
}
