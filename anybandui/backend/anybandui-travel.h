...
enum anybandui_travel_stage { TRAVEL_IDLE, TRAVEL_START, TRAVEL_APPROACH, TRAVEL_STEP, TRAVEL_ARRIVED, TRAVEL_DIGGING };
...
static void anybandui_capture_terrain_actions(cJSON *state)
{
 ...
 if(view) for(y=num(view,"y",0);y<num(view,"y",0)+num(view,"height",0);++y)
  for(x=num(view,"x",0);x<num(view,"x",0)+num(view,"width",0);++x) {
   cmd_code command=anybandui_terrain_command(loc(x,y));
   if(command!=CMD_NULL) {
    ...
    string(entry,"action",anybandui_terrain_action(command)); cJSON_AddItemToArray(actions,entry);
   }
  }
...
{
 size_t index;
 if(!square_in_bounds_fully(cave,grid) || grid.x>=anybandui_width || grid.y>=anybandui_height || !anybandui_cells) return false;
 if(anybandui_full_map) {
  struct map_visual visual;
  map_visual_readonly(grid,&visual);
  return visual.object_char!=0;
 }
 index=grid.y*anybandui_width+grid.x;
 return anybandui_cells_valid[index] && anybandui_cells[index].object_char!=0;
}
...
static void anybandui_travel_event(game_event_type type, game_event_data *data, void *user)
{
...
static bool anybandui_travel_continue(void)
{
 ...
 if(travel_level!=anybandui_level || player->is_dead || (player->timed[TMD_CONFUSED] &&
 ...
 if(adjacent) {
  if(!loc_eq(player->grid,travel_waypoint) || anybandui_terrain_command(travel_grid)!=travel_command) return false;
 ...
 if(!loc_eq(player->grid,travel_grid) || !anybandui_pickup_possible(travel_grid)) return false;
...
static void anybandui_travel_feedback(const char *label,bool active,bool interrupted)
{
...
static void anybandui_travel_begin(struct loc grid,cmd_code command)
{
 ...
 travel_display_grid=grid; travel_display_level=anybandui_level; travel_display_command=command;
 anybandui_travel_feedback(travel_display_attack?"Engaging adjacent creature":command==CMD_PICKUP?"Walking to pick up":command==CMD_TUNNEL?"Approaching wall to tunnel":command==CMD_GO_UP?"Walking to ascend":command==CMD_GO_DOWN?"Walking to descend":"Moving to destination",true,false);
}
static void anybandui_travel_finish(void)
{
 ...
 if((travel_display_command==CMD_GO_UP || travel_display_command==CMD_GO_DOWN) && travel_display_level!=anybandui_level)
 ...
 else if(travel_display_command==CMD_TUNNEL && anybandui_terrain_command(travel_display_grid)!=CMD_TUNNEL)
  ...
  anybandui_travel_feedback(travel_display_command==CMD_PICKUP?"Arrived: pickup attempted":"",false,false);
 else
  anybandui_travel_feedback(travel_stop_reason?travel_stop_reason:"",false,true);
}
