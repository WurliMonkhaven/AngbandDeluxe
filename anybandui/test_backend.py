...
class Engine:
    ...
    def fixture_command(self, key, answers=(), settle=True):
        """Drive upstream wizard input in a disposable test character only."""
        remaining=deque(answers)
        self.key(1); self.key(ord(key))
        for _ in range(50):
            self.call('state.get')
            if self.prompt:
                prompt=self.prompt; self.prompt=None
                value=True if prompt['type']=='confirmation' else remaining.popleft()
                old=self.state['revision']
                result=self.call('prompt.reply',{'prompt_id':prompt['prompt_id'],'value':value})
                assert 'result' in result,result
                self.next_state(old)
                if not remaining and not settle: return
            elif self.state['readiness']=='ready':
                assert not remaining, remaining
                return
            else: self.key('enter')
        raise AssertionError('Fixture command did not settle: '+self.screen())

    def fixture_effect(self, name, dice='0', subtype='0', settle=True):
        self.fixture_command('E',[name,str(dice),subtype,0,0,0,0],settle)
...
class BackendTests(unittest.TestCase):
    ...
    def test_fully_mapped_camera(self):
        ...
        explored_before=sum(bool(cell[8]) for row in e.state['dungeon']['cells'] for cell in row)
        e.key(1); e.key(ord('w'))
        for _ in range(20):
        ...
        self.assertGreater(sum(bool(cell[8]) for row in full['dungeon']['cells'] for cell in row),explored_before)
        for y,row in enumerate(full['dungeon']['cells']):
            for x,cell in enumerate(row):
                self.assertEqual(bool(cell[8]),bool(full['map']['known'][y][x]))
    ...
    def test_camera_hallucination_randomness(self):
        e=self.engine; e.hello(); e.birth()
        e.fixture_effect('TIMED_SET',100,'IMAGE')
        while e.state['readiness']!='ready': e.key('enter')
        e.call('session.close'); e.process.wait(timeout=10); e.stop()
    ...
    def test_native_keybindings(self):
        ...
        while e.state['readiness']!='ready': e.key('enter')
        data=e.call('keybindings.get')['result']
        blocked=Path(self.temp.name)/'user'/'anybandui-keybindings.json.tmp'
        blocked.mkdir()
    ...
    def test_confused_mouse_walk(self):
        ...
        e.hello(); e.birth()
        old = e.state['revision']
        e.fixture_effect('TIMED_SET',100,'CONFUSED')
        while e.state['readiness'] != 'ready': e.key('enter')
    ...
    def test_message_acknowledgement_keeps_dungeon(self):
        ...
        e.hello(); e.birth()
        before = e.state
        e.fixture_effect("DAMAGE",before["player"]["hp"]-1,settle=False)
    ...
    def setUp(self):
        ...
        self.temp = tempfile.TemporaryDirectory(prefix="anybandui-test-", dir=self.test_root)
    ...
    def test_no_developer_api(self):
        e=self.engine
        hello=e.call('hello',{'protocols':[{'major':0,'minor':1}]})['result']
        self.assertFalse(any(k.startswith('debug.') for k in hello['capabilities']))
        for method in ('damage','status','status.list','stats','experience','scene','glow_items','blast','breath','blink','quit'):
            self.assertIn('error',e.call('debug.'+method))
        e.birth()
        before=e.call('state.get')['result']
        for method in ('damage','status','stats','experience','scene','glow_items','blast','breath','blink','quit'):
            self.assertIn('error',e.call('debug.'+method))
        self.assertEqual(e.call('state.get')['result'],before)
    ...
    def test_run_summary_and_replay(self):
        ...
        old = e.state["revision"]
        e.fixture_effect("DAMAGE",30000,settle=False)
    ...
    def test_native_ball_spell_preview(self):
        e=self.engine
        e.hello(); e.birth(class_index=1)
        e.fixture_effect('GAIN_EXP',1000)
        for _ in range(20):
            if e.state['readiness']=='ready': break
            e.key('enter')
        self.assertEqual(e.state['readiness'],'ready',e.screen())
    ...
    def test_run_journal_persistence(self):
        ...
        self.assertFalse(any(v['kind']=='depth' for v in journal['entries']))
        e.key(ord('>'))
        while e.state['readiness']!='ready': e.key('enter')
        journal=e.call('journal.get')['result']
        self.assertEqual([v['depth'] for v in journal['entries'] if v['kind']=='depth'],[1])
    ...
    def test_blink_feedback(self):
        ...
        self.assertEqual(e.state['turn'],turn,'Fixture teleport costs no turn')
        blink=blinks[0]
    ...
    def test_fatal_psionic_messages_remain_native(self):
        ...
        lines = [line.replace("FORCE_DEPTH", "NEVER_BLOW") if line.startswith("flags:") else line for line in lines]
        lines.append("flags:NEVER_MOVE | NEVER_BLOW")
        ...
        e.hello(); e.birth()
        e.fixture_command('n',['Morgoth, Lord of Darkness'])
        final_statuses = False
        for _ in range(50):
            state = e.state
            if state["phase"] == "dead": break
            # Upstream wizard fixtures also show a separate score-exclusion screen.
            score_notice=state.get("messages",[{}])[0].get("text","").startswith("Score not registered")
            if state["player"]["hp"] < 0 and not score_notice:
                self.assertTrue(state.get("message_pending"), "Fatal spell messages must retain native continuation: " + str({k:v for k,v in state.items() if k in ("phase","readiness","messages","prompt")}) + " prompt=" +str(e.prompt))
        ...
        self.assertTrue(final_statuses, "Exercise the status messages after fatal damage: " + str(e.state.get("messages")))
        self.assertEqual(e.state["phase"], "dead")
