...
struct CharacterSelect {
 ...
 static int roster_actions(bool enabled) {
  ...
  for(int i=0;i<2;++i) {
   ...
   const auto ink=ImGui::GetColorU32(i?AnybandUITheme::green():ImGui::GetStyleColorVec4(ImGuiCol_TextDisabled));
 ...
 int draw(const json &saves,bool enabled) {
  ...
  if(saves.empty()) {
   emblem(font*11,ImGui::GetColorU32(AnybandUITheme::green())); ImGui::Spacing();
