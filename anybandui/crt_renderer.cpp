...
void CrtRenderer::draw_ui(SDL_GPUCommandBuffer *cmd,SDL_GPUTexture *target,ImDrawData *data,bool clear,int first,int last) {
 ...
 AnybandUIGpuRenderRange(data,cmd,pass,first,last<0?data->CmdListsCount:last); SDL_EndGPURenderPass(pass);
}
...
void CrtRenderer::render_scene(SDL_GPUCommandBuffer *cmd,SDL_GPUTexture *destination,Uint32 width,Uint32 height,ImDrawData *data,const CrtFrame &frame) {
 ...
 u.surface[0]=settings.level(Dots); u.surface[1]=settings.level(Interference); u.surface[2]=float(settings.mask); u.surface[3]=0;
