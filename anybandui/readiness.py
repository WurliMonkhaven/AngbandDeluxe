...
def main():
    ...
    build = ROOT/"build-anybandui-native"
    game = build/"game"
    output = ROOT/"build-anybandui"/("readiness-"+datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S"))
    output.mkdir()
    records = []
    ...
    if not args.skip_build:
        run("build",python+[ROOT/"anybandui/build.py","--ninja","--target","OurExecutable","AnybandUI",
                            "anybandui-client-tests","anybandui-gpu-tests","anybandui-audio-tests","anybandui-transport-tests","anybandui-ui-preview","anybandui-window-tests"],timeout=600)
    run("client",[game/"anybandui-client-tests.exe",output/"settings.json"])
    run("backend",python+[ROOT/"anybandui/test_backend.py","--backend",game/"angband-backend.exe"])
    run("audio",[game/"anybandui-audio-tests.exe",game/"audio"])
    run("gpu",[game/"anybandui-gpu-tests.exe","direct3d12"])
    run("frame-pacing",[game/"anybandui-gpu-tests.exe","direct3d12","--bench"])
    ...
    run("detached-windows",[game/"anybandui-window-tests.exe",output/"dungeon-ui.json"])
    if args.ui_fixture:
        for kind in ("layout", "inventory"):
            run(kind+"-interaction",python+[ROOT/f"anybandui/test_{kind}_ui.py","--preview",game/"anybandui-ui-preview.exe",
    ...
    try:
        ...
        probe.stop()
    run("transport",[game/"anybandui-transport-tests.exe",game/"angband-backend.exe",ROOT/"lib",fixture])
    if args.package:
        ...
        try:
            ...
            backup.rename(font)
        run("packaged-journeys",python+[ROOT/"anybandui/test_backend.py","--backend",relocated/"angband-backend.exe",
        ...
        run("packaged-audio",[game/"anybandui-audio-tests.exe",relocated/"audio"],cwd=output)
