...
struct WorkspaceLayout {
 ...
 void source(int panel) {
  if(editing && ImGui::BeginDragDropSource()) {
   ImGui::SetDragDropPayload("ANYBANDUI_PANEL",&panel,sizeof(panel)); ImGui::TextUnformatted(names[panel]); ImGui::EndDragDropSource();
  }
 }
 ...
 void target(int id,int edge,ImVec2 pos,ImVec2 size,const Enabled &enabled) {
  if(ImGui::BeginDragDropTarget()) {
   if(const auto *payload=ImGui::AcceptDragDropPayload("ANYBANDUI_PANEL",ImGuiDragDropFlags_AcceptBeforeDelivery|ImGuiDragDropFlags_AcceptNoDrawDefaultRect)) {
